    ...
    UtilityNS::RelativeInfo& info)
{
    ...
    if (trajectory.size() == 2) {
        ...
        if (info.iFront > 0)
            info.iBack = info.iFront - 1;  //iBack是iFront的前一个点
        else
            info.iBack = 0;
...
void RolloutGenerator::run(tf::StampedTransform transform, nav_msgs::Path pathMsg, nav_msgs::Path& fixedGlobal, nav_msgs::Path& centerPath, nav_msgs::Path& remainingPath, vector<nav_msgs::Path>& alternativePaths)
{
    ...
    // 获取采样后的路径点（rolloutNumber + 1 条路径的点）
 ...
 * @brief 生成候选rollouts
 * 
 * @param referencePaths 参考路径的vector
 * @param carPos 当前位姿
 * @param speed 当前速度 m/s
 * @param microPlanDistance 
 * @param carTipMargin 
 * @param rollInMargin 
 * @param rollInSpeedFactor 
 * @param pathDensity 
 * @param rollOutDensity 
 * @param rollOutNumber 
 * @param SmoothDataWeight 
 * @param SmoothWeight 
 * @param SmoothTolerance 
 * @param rollOutsPaths 
 * @param sampledPoints_debug 
 */
...
/**
 * @brief 由中心轨迹的采样点计算候选rollouts的采样点并平滑
 * 
 * @param carPos 位置
 * @param speed 速度
 * @param originalCenter 中心轨迹
 * @param start_index 
 * @param end_index 
 * @param end_laterals 
 * @param rollInPaths 
 * @param max_roll_distance 
 * @param carTipMargin 开始路径平滑的距离
 * @param rollInMargin 从车头发散的距离
 * @param rollInSpeedFactor 
 * @param pathDensity 
 * @param rollOutDensity 
 * @param rollOutNumber 
 * @param SmoothDataWeight 
 * @param SmoothWeight 路径平滑的权重
 * @param SmoothTolerance 路径平滑的阈值
 * @param sampledPoints 最后的采样点
 */
    ...
    vector<UtilityNS::WayPoint>& sampledPoints)
{
    ...
    // 横向扩展的点的列表
    // 横向扩展距离 = rollOutNumber / 2 * rollOutDensity
    ...
    rollInPaths.clear();
    vector<double> inc_list; // 每个横向点的增量
    vector<double> inc_list_inc;
    for (int i = 0; i < rollOutNumber + 1; i++) {
        rollInPaths.push_back(vector<UtilityNS::WayPoint>()); // 生成rollOutNumber + 1个空的路径
    ...
    // Step: 2. 生成rollInPaths
    // Step: 2.1 每个路径都生成 close ~ smoothing_start_index 的路径点 (smoothing_start_index = 5)

    // 开始阶段，close index到smoothing_start_index
    // start section
    //Insert First strait points within the tip of the car range
    for (unsigned int j = start_index; j < smoothing_start_index; j++) {
        // 对于每一个点，生成初始的rollOutNumber + 1个路径点
    ...
    // Step: 2.2 每个路径都生成 smoothing_start_index ~ end_index 的路径点
    // 长度为 start_distance = rollInSpeedFactor * speed + rollInMargin
    // transition section
    ...
    // Step: 2.3 每个路径都生成 end_index ~ smoothing_end_index 的路径点
    ...
    // Step: 2.4 将execluded path 插入到 rollInPaths的开头
    for (unsigned int i = 0; i < rollOutNumber + 1; i++) {
    ...
    // Step: 2.5 每个路径生成 smoothing_end_index ~ originalCenter.size() 的路径点
    ...
    // // tixiao's processing
    // Step: 3. 平滑路径
    // Step: 3.1 将路径最后的seg_num_2 (5) 个点设置为原始路径的点
 ...
 * 
 * @param path 需要平滑的路径
 * @param weight_data 前后路径的权重
 * @param weight_smooth 上次平滑的权重
 * @param tolerance 平滑增量的阈值
 */
    ...
    double weight_smooth, double tolerance)
{
    ...
    while (change >= tolerance) {
        // 双层循环，每次循环都对整个路径进行一次平滑，知道变化量change小于tolerance
        change = 0.0;
    ...
    vector<UtilityNS::WayPoint>& extractedPath)
{
    ...
    // 将close_index之前2m内的路径点插入到extractedPath中
    // close_index -> 0
    ...
    // 将close_index+1 之后 minDistance 内的路径点插入到extractedPath中
    // close_index -> end
    dis = 0;
